    ...
    <div class="page-inner">
        <div class="page-header">
            ...
            <ul class="breadcrumbs">
                ...
                <li class="nav-item">
                    <a href="javascript:void()">Out Stock</a>
                </li>
                    ...
                    <div class="card">
                        <div class="card-header">
                            <div class="align-items-center">
                                <div class="row">
                                    <div class="col-sm-6">
                                        <h4 class="card-title">Stock Keluar</h4>
                                    </div>
                                    <div class="col-sm-6">
                                        <div class="float-end">
                                            <a onclick="add_Stock()"
                                            ...
                                            <button onclick="Print_data_stock()" type="button"
                                                class="btn btn-warning btn-round btn-sm m-2"><i
                                                    class="fa fa-print me-2"></i>Print Stock Keluar</button>
                                            <button onclick="export_pdf_stock_keluar()" type="button"
                                                class="btn btn-danger btn-round btn-sm m-2"><i
                                                    class="fa fa-file-pdf me-2"></i>PDF Stock Keluar</button>
                                ...
                                <table id="myTables" class="table table-sm table-hover " style="width: 100%;">
                                    <thead>
                                        <tr>
                                            ...
                                            <th scope="col">Tanggal Keluar Stock</th>
        ...
        <div class="modal-content">
            ...
            <form id="form">
                <div class="modal-body">
                    ...
                    <div class="row mb-3">
                        <label for="stock_keluar" class="col-sm-2 col-form-label">Stock</label>
                        <div class="col-sm-10">
                            <input type="number" name="stock_keluar" class="form-control" id="stock_keluar">
                        </div>
                    </div>
...
function getData() {
    ...
    new DataTable('#myTables', {
        ...
        order: [],
        ajax: {
            "url": "<?php echo site_url('admin/stock/dataTableStockout') ?>",
            "type": "POST",
            "data": {
                bulan: $('#tgl').val(),
                tahun: $('#year').val(),
...
function Print_data_stock() {
    bulan = $('#tgl').val();
    tahun = $('#year').val();
    window.location = '<?php echo base_url('admin/stock/print_stock_keluar/') ?>' + bulan + '/' + tahun;
}


function export_pdf_stock_keluar() {
    bulan = $('#tgl').val();
    tahun = $('#year').val();
    window.location = '<?php echo base_url('admin/stock/export_pdf_stock_keluar/') ?>' + bulan + '/' + tahun;
}
...
$('#form').submit(function(e) {
    ...
    if ($('[name="stock_keluar"]').val() == '') {
        toastr.info('Stock cannot be empty')
        return false;
    }
    if ($('[name="tanggal_keluar"]').val() == '') {
        toastr.info('Tanggal cannot be empty')
        return false;
    }
    ...
    if (save_method == 'add') {
        url = "<?php echo site_url('admin/stock/insertOutStock/') ?>";
    } else {
        url = "<?php echo site_url('admin/stock/updateOutStock/') ?>";
    }
    $.ajax({
        url: url,
        type: "POST",
...
function edit_stock_keluar(id) {
    ...
    $.ajax({
        url: "<?php echo site_url('admin/stock/get_data_out_stock_ById/') ?>" + id,
        type: "POST",
        dataType: "JSON",
        ...
        success: function(data) {
            ...
            $('[name="old_stock"]').val(data.stock_keluar);
            $('[name="stock_keluar"]').val(data.stock_keluar);
...
function deletes(id) {
    Swal.fire({
        ...
        if (result.isConfirmed) {
            $.ajax({
                url: "<?php echo site_url('admin/stock/delete_data_out_stock/') ?>" + id,
                type: "POST",
                dataType: "JSON",
                success: function(data) {
                    if (data.status == '00') {
